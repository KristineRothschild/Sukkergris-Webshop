//klasser samme som sist oppgave, klasse for alt inne på nettside
export class categoryView extends HTMLElement { //skal vise kategorier i kategori view
  ...
  refresh(dataArr){
    ...
    categoryView.addEventListener("click", () => {
      const plantSelected = new CustomEvent("plantSelected", {composed: true, bubbles: true, detail: value,});
...
export class plantView extends HTMLElement { //skal ta deg videre til plantview
...
//----------------------------------------------
  ...
  //----------------------------------------------
  backClick(evt){
    const plantBack = new CustomEvent("plantBack", {composed: true, bubbles: true});
...
export class plantDetails extends HTMLElement {
  constructor() {
    ...
    backBtn.addEventListener('click', () => {
      const detailsBack = new CustomEvent("detailsBack", { composed: true, bubbles: true });
  ...
  refresh(data) {
    ...
    this.wrapper.innerHTML = `
      <img src="http://sukkergris.no/plantimages/large/${data.bildefil.replace('.png', '.jpg')}" alt="${data.navn}" />
      <h2>${data.navn}</h2>
